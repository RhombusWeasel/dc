        ...
        */
        ...
        templates: {
            ...
            iterate: function(data, changes) {
                console.log('DC : utils.tools.templates.iterate : ', data, changes);
                for (let i = 0; i < changes.length; i++) {
                    if (utils.tools.templates.operations?.[changes[i].mode]) utils.tools.templates.operations[mode](data, changes[i]);
                }
                return data;
            },
            modify: function(data, changes) {
    ...
    system: {
        new: {
            entity: function(changes) {
                let char = {};
                char = utils.tools.templates.iterate(char, utils.templates.entity);
                char = utils.tools.templates.iterate(char, utils.templates.stats);
                char = utils.tools.templates.iterate(char, utils.templates.pools);
                char = utils.tools.templates.iterate(char, utils.templates.skills);
                return char;
            },

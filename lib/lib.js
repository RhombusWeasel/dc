utils = {
        ...
        templates: {
            operators: {
                ...
                select: function(key, value) {
                    return {mode: 'ensure', path: value.select, type: 'select', value: value.default, select: value.select, action: 'add'};
                },
            },
            build: function(data) {
                let tmp = [];
                for (const key in data) {
                    if (Object.hasOwnProperty.call(data, key)) {
                        ...
                        if (utils.tools.templates.operators?.[t]) tmp.push(utils.tools.templates.operators[t](key, value));
                    }
                }
                return tmp
            },
            operations: {
                ...
                modifiers: {
                    ...
                    add: function(data, changes) {
                        return utils.tools.path.set(data, changes.path, utils.tools.path.get(data, changes.path) + changes.value);
                    },
            ...
            iterate: function(data, changes) {
                ...
                if (data_type == 'list') {
                    for (let i = 0; i < changes.length; i++) {
                        ...
                        if(change?.trigger && change.trigger != 'permanent') {
                            utils.tools.path.set(data, `${change.trigger}.${utils.tools.safe_key(change.label)}`, change);
                            if (change?.expires_when) utils.tools.path.set(data, `expiry.${utils.tools.path.split(change.expires_when).key}.${utils.tools.safe_key(change.label)}`, change);
                        }else{
            ...
            modifiers: {
                operations: {
                    ...
                    pool: function(data) {
                        return {mode: 'ensure', path: data.path, value: data, label: data.label, descriptions: data.descriptions, image: data.image};
                    },
                    modify_pool: function(data) {
                        return {mode: 'modify', action: 'add', path: `${data.path}.value`, trigger: data.trigger, value: data.value, label: data.label, descriptions: data.descriptions, image: data.image};
                    },
                    increase_pool: function(data) {
                        // ADD THIS FOR INCREASING POOL MAXIMUMS
                    },
    ...
    system: {
        new: {
            template: function(type, changes) {
                let data = utils.tools.templates.iterate({}, utils.templates[type]);
                data.type = type;
                return utils.tools.templates.modify(data, changes);
            },
            entity: function() {
                var e = utils.tools.templates.iterate({}, utils.templates.entity);
                utils.tools.templates.iterate(e, utils.templates.stats);
                utils.tools.templates.iterate(e, utils.templates.pools);
                utils.tools.templates.iterate(e, utils.templates.skills);
                utils.tools.templates.iterate(e, utils.templates.spells);
                utils.tools.templates.iterate(e, utils.templates.triggers);
                return e;
            },
            modifier: function(index, changes) {
                let mod = utils.tools.templates.iterate({}, utils.templates.modifiers[index].template);
